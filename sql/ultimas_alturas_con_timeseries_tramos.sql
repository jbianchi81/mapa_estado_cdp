create view ultimas_alturas_con_timeseries_tramos AS
...
WITH alturas_last AS (
         SELECT series.estacion_id AS unid,
           ...
           FROM estaciones_view,
            ...
            valores_num,
            tramos
          WHERE estaciones_view.sitecode=series.estacion_id AND series.id = observaciones.series_id AND observaciones.id = valores_num.obs_id AND series.var_id = 2 AND series.proc_id = 1 AND series.unit_id = 11 
	...
	AND observaciones.timeend <= case when '1800-01-01' = '1800-01-01' 
	      ...
	      AND estaciones_view.redid= CASE WHEN 0!=0 THEN 0 ELSE estaciones_view.redid END 
	      AND estaciones_view.tipo= CASE WHEN 'X' != 'X' THEN 'X' else estaciones_view.tipo END
        AND tramos.unid=estaciones_view.sitecode
        ), ult AS (
